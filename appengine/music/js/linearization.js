...
Blockly.Linearization.prototype.makeInputListItem_ = function(node) {
  ...
  switch (node.getType()) {
    case Blockly.ASTNode.types.FIELD:
      ...
      if (location instanceof Blockly.FieldPitch) {
...
Blockly.Linearization.prototype.makePitchItem_ = function(pitchField) {
  var options = Blockly.FieldPitch.NOTES;
  var value = 0;
  var entry = options[0];
  for (var i = 0, option; option = options[i]; i++) {
    if (option[i] === pitchField.getText()) {
      entry = options[i];
      value = i;
      break;
    }
